...
class PostTest extends TestCase {
    ...
    {
        try {
            $res = $this->client->request('POST', 'http://localhost:8000/patients', [
                'form_params' => ['test' => 1]
            ]);
            $this->assertEquals($res->getStatusCode(), '200');
            $this->assertEquals(['body' => ['test' => 1]], json_decode($res->getBody()->getContents(), true));
    ...
    {
        try {
            $res = $this->client->request('POST', 'http://localhost:8000/patients/1', [
                'json' => ['test' => 1]
            ]);
