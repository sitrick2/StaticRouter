...
class PatchTest extends TestCase {
    ...
    {
        try {
            $res = $this->client->request('PATCH', 'http://localhost:8000/patients/1/metrics/abc', [
                'json' => ['test' => 1]
            ]);
            $this->assertEquals($res->getStatusCode(), '200');
            $this->assertEquals(['body' => ['test' => 1], 'patient_id' => '1', 'metrics_id' => 'abc'], json_decode($res->getBody()->getContents(), true));
    ...
    {
        try {
            $res = $this->client->request('PATCH', 'http://localhost:8000/patients/1/metrics', [
                'json' => ['test' => 1]
            ]);
